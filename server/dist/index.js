...
import { currencies, exchangeRates, settings, materials, products, billOfMaterials, intermediateMaterialBom, materialPriceHistory, priceLevels, priceLevelItems, customers, specialPricing, priceLists, priceListItems, activityLog } from './schema';
...
function parseSpecialPricingOverrideType(value) {
...
async function buildSpecialPricingResponse(row) {
...
async function saveCustomerSpecialPricing(req, res) {
    const customerId = parseInt(req.params.id);
    if (!Number.isInteger(customerId)) {
        return res.status(400).json({ error: 'Invalid customer id' });
    }
