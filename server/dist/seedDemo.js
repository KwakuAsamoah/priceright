...
export async function seedDemoData(options) {
    ...
    if (!forceSeed) {
        try {
            const existingLevels = db.prepare('SELECT COUNT(*) AS count FROM price_levels').get();
    ...
    db.exec(schemaSql);
    const insertCurrency = db.prepare('INSERT INTO currencies (code, name, symbol, is_active, created_at) VALUES (?, ?, ?, 1, ?)');
    const insertRate = db.prepare('INSERT INTO exchange_rates (currency_id, rate_to_base, effective_date, source, created_at) VALUES (?, ?, ?, ?, ?)');
...
`);
    const insertIntermediateBom = db.prepare('INSERT INTO intermediate_material_bom (intermediate_material_id, component_material_id, quantity, created_at) VALUES (?, ?, ?, ?)');
...
`);
    const insertBom = db.prepare('INSERT INTO bill_of_materials (product_id, material_id, quantity, created_at) VALUES (?, ?, ?, ?)');
    const insertPriceLevel = db.prepare('INSERT INTO price_levels (name, multiplier, adjustment_type, adjustment_percentage, description, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?)');
    const insertCustomer = db.prepare('INSERT INTO customers (name, price_level_id, created_at, updated_at) VALUES (?, ?, ?, ?)');
...
`);
    const insertPriceList = db.prepare('INSERT INTO price_lists (name, customer_id, price_level_id, valid_from, valid_until, status, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
    const insertPriceListItem = db.prepare('INSERT INTO price_list_items (price_list_id, product_id, base_price, discount_percentage, final_price, price_source, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    const insertActivity = db.prepare('INSERT INTO activity_log (action, entity_type, entity_id, entity_name, performed_by, details, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)');
    const txn = db.transaction(() => {
        ...
        for (const [name, sku, description, category, unit, bulkQuantity, bulkPrice] of primaryMaterials) {
            ...
            insertMaterial.run(name, sku, description, 'primary', category, unit, bulkQuantity, bulkPrice, 1, bulkPrice, bulkPrice, unitPrice, 0, 0, 'yield', 100, unitPrice, 'Demo Supplier', now, now);
        }
        ...
        const intermediateId = Number(insertMaterial.run('Chocolate Syrup Base', 'INT-001', 'Intermediate blend for toppings and fillings', 'intermediate', 'Intermediate', 'kg', intermediateBulkQty, intermediateBulkPrice, 1, intermediateBulkPrice, intermediateBulkPrice, intermediateUnitPrice, 5, 8, 'yield', 98, intermediateUnitPrice, 'Demo Kitchen', now, now).lastInsertRowid);
        ...
        for (const [name, sku, description, category, overhead, margin, otherCosts, productionMode, batchYield, approvedPrice] of productRows) {
            const result = insertProduct.run(name, sku, description, category, overhead, margin, otherCosts, productionMode, batchYield, approvedPrice, 'approved', approvedPrice, 'demo-admin', now, new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(), now, now);
        ...
        const priceLevelIdByName = new Map();
        ...
        for (const [levelName, adjustmentPercentage] of ruleLevelAdjustments) {
            ...
            for (const product of insertedProducts) {
                insertPriceLevelItem.run(levelId, product.id, 'rule_discount', adjustmentPercentage, null, 'approved', 'Admin', now, 'Seeded level rule', now, now);
            }
        }
    ...
    txn();
    const counts = {
        currencies: db.prepare('SELECT COUNT(*) AS count FROM currencies').get(),
        materials: db.prepare('SELECT COUNT(*) AS count FROM materials').get(),
        products: db.prepare('SELECT COUNT(*) AS count FROM products').get(),
        price_levels: db.prepare('SELECT COUNT(*) AS count FROM price_levels').get(),
        price_level_items: db.prepare('SELECT COUNT(*) AS count FROM price_level_items').get(),
        customers: db.prepare('SELECT COUNT(*) AS count FROM customers').get(),
        special_pricing: db.prepare('SELECT COUNT(*) AS count FROM special_pricing').get(),
        price_lists: db.prepare('SELECT COUNT(*) AS count FROM price_lists').get(),
        activity_log: db.prepare('SELECT COUNT(*) AS count FROM activity_log').get(),
    };
