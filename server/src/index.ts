...
function runSyncTransaction(work: (tx: DbExecutor) => void): void {
...
type PriceLevelCurrencyContext = {
...
async function resolvePriceLevelCurrencyContext(
  level: typeof priceLevels.$inferSelect,
): Promise<PriceLevelCurrencyContext> {
...
async function enrichPriceLevelRow(level: typeof priceLevels.$inferSelect) {
...
function parseOptionalCurrencyId(value: unknown): number | null | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (value === null || value === '') {
...
function safeBatchYield(batchYield: unknown): number {
...
function parsePositiveExchangeRate(rateToBase: unknown): number | null {
...
async function resolveExchangeRateForPurchaseCurrency(
  purchaseCurrencyId: number,
  baseCurrency: { id: number },
): Promise<number> {
  ...
  if (rateRows.length === 0) {
    throw new Error(
      `No exchange rate found for currency ${currencyCode}. Set an exchange rate in Settings before saving materials in this currency.`,
    );
  }
  ...
  if (exchangeRate == null) {
    throw new Error(
      `Invalid exchange rate for currency ${currencyCode}. Set a valid exchange rate in Settings before saving materials in this currency.`,
    );
  }
...
function isExchangeRateValidationError(error: unknown): error is Error {
  return error instanceof Error && (
    error.message.includes('No exchange rate found for currency')
...
async function toPriceLevelItemResponse(
  ...
  const hasConvertedCurrency = Boolean(
    currencyContext.currencyCode && currencyContext.currencyId,
  );
...
async function getNeedsReviewIncreaseThreshold(): Promise<number> {
  try {
    const rows = await getActiveDb()
      .select({ settingValue: settings.settingValue })
      .from(settings)
...
type MaterialInUseDeleteError = {
  error: string;
  code: 'MATERIAL_IN_USE';
  details?: Record<string, unknown>;
};

async function getMaterialInUseDeleteError(
  materialId: number,
  materialName: string,
): Promise<MaterialInUseDeleteError | null> {
...
type ProductDeleteBlockError = {
  error: string;
  code: 'PRODUCT_IN_USE';
  details?: Record<string, unknown>;
};

async function getProductPriceLevelDeleteError(productId: number): Promise<ProductDeleteBlockError | null> {
  ...
  const uniquePriceLevelNames = Array.from(
    new Set(priceLevelRows.map((row) => String(row.priceLevelName || '').trim()).filter(Boolean)),
  );
  ...
  try {
    const rows = await db
      .select({
        ...
        bomUsageCount: sql<number>`count(${billOfMaterials.id})`.mapWith(Number),
        totalContribution: sql<number>`sum(${billOfMaterials.quantity} * ${materials.unitPrice})`.mapWith(Number),
      })
      .from(billOfMaterials)
      .innerJoin(products, eq(billOfMaterials.productId, products.id))
...
function parseVolatilityPeriodDays(period: unknown): number {
...
function getMaterialCostAtPeriodStart(
  historyAsc: Array<{ changedAt: unknown; priceInBaseCurrency: number | null }>,
  periodStart: Date,
): number | null {
...
app.get('/api/reports/price-volatility', async (req, res) => {
  try {
    ...
    const historyByMaterialId = new Map<number, Array<{ changedAt: unknown; priceInBaseCurrency: number | null }>>();
    ...
    for (const material of activeMaterials) {
      ...
      if (historyAsc.length === 0) continue;
    ...
    const biggestDecrease = volatilityRows.reduce<typeof volatilityRows[number] | null>((best, row) => {
      if (!best || row.changePercent < best.changePercent) return row;
      return best;
    }, null);
...
app.delete('/api/intermediate-materials/bulk', async (req, res) => {
  try {
    ...
    const blocked: Array<MaterialInUseDeleteError & { materialId: number; materialName: string }> = [];
...
async function hasApprovedPriceLevelReference(productId: number): Promise<boolean> {
  const rows = await getActiveDb()
...
async function getStaleApprovedCustomPriceLevelItems(productId: number): Promise<Array<{
  priceLevelId: number;
...
app.post('/api/products/bulk-approve', async (req, res) => {
  try {
    ...
    const prepResults = await Promise.all(productIds.map(async (productId): Promise<BulkApprovePrepResult> => {
      const bomItems = await db
        .select({ id: billOfMaterials.id })
        .from(billOfMaterials)
        .where(eq(billOfMaterials.productId, productId));
...
app.delete('/api/products/bulk', async (req, res) => {
  try {
    ...
    const blocked: Array<ProductDeleteBlockError & { productId: number; productName: string }> = [];
...
app.put('/api/price-level-rules/:id', async (req, res) => {
  try {
    ...
    const updateValues: Partial<typeof priceLevels.$inferInsert> = {
      name,
      multiplier,
      adjustmentType,
      adjustmentPercentage: numericPercentage,
...
if (userDataPath) {
  possibleTemplateDirs.push(
    path.join(userDataPath, '..', 'app', 'client-dist', 'templates')
  );
}
