  ...
  `);
    ...
    const txn = db.transaction(() => {
        ...
        for (let idx = 0; idx < prodDefs.length; idx++) {
            ...
            const prodId = Number(insProd.run(pd.name, pd.sku, pd.desc, pd.cat, pd.overhead, targetMarkup, pd.otherDirect, pd.mode, pd.batchYield, approvedPrice ?? 0, pd.approved ? 'approved' : 'pending', approvedPrice, pd.approved ? 'Admin' : null, approvedAt, null, now, now).lastInsertRowid);
