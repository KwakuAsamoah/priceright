...
function priceForMarkupOnCost(productionCostPerUnit, markupOnCostPct) {
...
export async function seedDemoData(options) {
    ...
    if (!options?.force) {
        try {
            const row = db.prepare('SELECT COUNT(*) AS cnt FROM price_levels').get();
  ...
  `);
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?)
  `);
    const insPackSize = db.prepare('INSERT INTO price_level_pack_sizes (price_level_item_id, pack_quantity, created_at) VALUES (?,?,?)');
    ...
    const txn = db.transaction(() => {
        ...
        for (const [name, sku, desc, cat, unit, bulkQty, bulkPrice, curId, supplier] of rawDefs) {
            ...
            rawIds.push(Number(insMat.run(name, sku, desc, 'primary', cat, unit, bulkQty, bulkPrice, curId, bulkPrice, priceGHS, unitPrice, 0, 0, 'yield', 100, unitPrice, supplier, now, now).lastInsertRowid));
        }
        // Helper: raw material DB id by 0-based index into rawDefs
        const rId = (i) => rawIds[i];
        // Helper: unit price (GHS) for a raw material by index
        const rawUP = (i) => {
            const [, , , , , bulkQty, bulkPrice, curId] = rawDefs[i];
            const g = curId === usdId
                ? bulkPrice * USD_TO_GHS
                : bulkPrice;
            return g / bulkQty;
        };
        const intDefs = [
        ...
        for (const def of intDefs) {
            ...
            const intDbId = Number(insMat.run(def.name, def.sku, def.desc, 'intermediate', def.category, def.unit, totalInput, totalCost, ghsId, totalCost, totalCost, calcCost, def.overhead, def.marginPct, 'yield', def.yieldPct, calcCost, 'Demo Kitchen', now, now).lastInsertRowid);
            intIds.push(intDbId);
        ...
        for (let idx = 0; idx < prodDefs.length; idx++) {
            ...
            const prodId = Number(insProd.run(pd.name, pd.sku, pd.desc, pd.cat, pd.overhead, targetMarkup, 0, pd.otherDirect, pd.mode, pd.batchYield, approvedPrice ?? 0, pd.approved ? 'approved' : 'pending', approvedPrice, pd.approved ? 'Admin' : null, approvedAt, null, now, now).lastInsertRowid);
        ...
        const averageApprovedMarkup = approvedMarkupPercents.length > 0
            ? Math.round(approvedMarkupPercents.reduce((sum, markup) => sum + markup, 0) / approvedMarkupPercents.length * 10) / 10
            : 0;
        // ────────────────────────────────────────────────────────────────────────
        // 3 PRICE LEVELS
        // ────────────────────────────────────────────────────────────────────────
        const levelDefs = [
            { name: 'Standard Retail', multiplier: 1.00, adjType: 'discount', adjPct: 0, desc: 'Base retail prices for direct and walk-in customers' },
            { name: 'Wholesale Clients', multiplier: 0.85, adjType: 'discount', adjPct: 15, desc: 'Volume buyers — 15% discount on approved base prices' },
            { name: 'Export Partners', multiplier: 0.80, adjType: 'discount', adjPct: 20, desc: 'International distribution — 20% export discount' },
        ];
        const levelIds = [];
        for (const ld of levelDefs) {
            levelIds.push(Number(insLevel.run(ld.name, ld.multiplier, ld.adjType, ld.adjPct, ld.desc, now, now).lastInsertRowid));
        }
        ...
        for (let li = 0; li < levelIds.length; li++) {
            ...
            for (const { pd, prodId } of approvedEntries) {
                const itemId = Number(insLvlItem.run(levelId, prodId, 'rule_discount', adjPct, null, 'approved', 'Admin', now, 'Demo seed — auto-approved', now, now).lastInsertRowid);
